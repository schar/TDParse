...
view model =
  HE.div [HA.id "parser"]
    ...
    , HE.div "content"
      [ HE.div "parses" $
          -- map (\p -> HE.div [HA.class' "parse"] p) $
          --   fromMaybe [HE.text "No parse"] $ model.currentProofs <#> (map displayProof <<< filter model.typeOfInterest)
      ...
      , HE.div [HA.id "lexicon", HA.style {visibility: if snd (model.lex) then "visible" else "collapse"}] $
          ...
          <>
          map (\l -> HE.p [HA.class' "lexitem"] $ displayLexItem l) (fst model.lex)
      ]
    ]
...
displayLexItem (s ^ w) =
  HE.text (s <> ": ") <>
  intercalate (HE.text "; ")
  (fromFoldable w <#> \(_ ^ _ ^ ty) -> HE.span [HA.class' "type"] $ displayTy ty)

-- | Mount the application on the given selector
main :: Effect Unit
main = mount_ (QuerySelector "#home")
       { init: init ^ [],
         subscribe: []
       , update
       , view
